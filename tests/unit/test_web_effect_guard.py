...
def test_a_stale_heartbeat_falls_back_to_the_configured_level(web, conn, layout, running_daemon):
    """There is nothing to disagree with. Refusing on the strength of a stale claim would
    be the same class of surprise in the other direction."""
    layout.heartbeat_path.write_text(
        '{"ts":"2020-01-01T00:00:00Z","pid":1,"effect_level":"plan","activity":"idle",'
        '"cycles":1}',
        encoding="utf-8",
    )
    item_id = seed_item(conn, state="failed")
    assert web.get_json("/active").json()["effect_mismatch"] is None
    # Refused for the *right* reason — the blocking condition — not for a level mismatch.
    response = web.post_json(f"/item/{item_id}/retry")
    assert "EFFECT LEVEL MISMATCH" not in response.text


def test_an_absent_heartbeat_is_not_a_mismatch(web, conn, layout):
