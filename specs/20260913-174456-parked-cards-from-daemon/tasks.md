...
- [X] T001 [P] Add `ignore_lists: list[str] | None = None` to `Heartbeat` and an `ignore_lists`
...
- [X] T002 In `src/robot_army/health.py`, factor `published_cap`'s running/pid-match checks into a
...
- [X] T003 [P] Tests H1–H2 and B1–B6 in `tests/unit/test_health.py`, beside the issue #30 block,
  reusing `report_with`/`write_at`; confirm the existing `published_cap` tests pass unchanged
- [X] T004 Publish `ignore_lists=list(self.config.trello.ignore_lists) if self.config.trello else None`
  from `Daemon._heartbeat` in `src/robot_army/daemon.py`, with a comment naming issue #74
- [X] T005 [P] Tests H3–H4 (the daemon's own heartbeat carries `["Icebox"]` with a board, `null`
...
- [X] T006 In `src/robot_army/operations.py`: change `card_is_parked(card, config)` to
...
- [X] T007 [P] Tests R1–R4 for `resolve_ignore_lists` and the mismatch sentence in
...
- [X] T008 [US1] `operations.cards(..., published_ignore_lists=_OWN_READING)` in
...
- [X] T009 [US1] In `src/robot_army/web/server.py`, compute
...
- [X] T010 [US1] `pages.cards_view(ctx, *, include_simulated, published_ignore_lists=...)` in
...
- [X] T011 [US1] *(Landed in `tests/unit/test_ignored_lists.py`, beside the existing
  web-surface tests and their `track`/`listing_context` helpers; the `web` and
  `running_daemon` fixtures are conftest's, so nothing needed `test_web_views.py`.)*
  Tests L1–L3 and L6 in `tests/unit/test_web_views.py`, through the `web` harness with
...
- [X] T012 [US2] `_card_for_item` in `src/robot_army/operations.py` uses
  `resolve_ignore_lists(ctx.config, _published_ignore_lists(ctx))[0]` (research R4), reading only
  when a card was found
- [X] T013 [US2] `operations.cards` prints `ignore_list_disagreement` after the table when present,
  in `src/robot_army/operations.py`
- [X] T014 [P] [US2] Tests L4–L5 in `tests/unit/test_ignored_lists.py` using `running_daemon`,
...
- [X] T015 [P] [US2] Update the existing call sites and tests of `card_is_parked` to the new
...
- [X] T016 [US3] *(Landed in `tests/unit/test_ignored_lists.py` with T011.)* Assert in
  `tests/unit/test_web_views.py` that no mismatch banner appears on
...
- [X] T017 [P] `docs/guide/state.md`: the heartbeat also carries `ignore_lists`, why, the `jq`
  line, and that an older heartbeat or `null` is *not published*
- [X] T018 [P] `docs/guide/2-intake.md` "Parking a card": parkedness on both listings is the running
  daemon's decision, and the `IGNORE LIST MISMATCH` line and what to restart
- [X] T019 Run `uv run pytest` and `uv run ruff check` (if configured in `pyproject.toml`); all pass
  *(4090 passed, 1 skipped — the documented manual Trello round. `ruff check src/ tests/`,
  which is CI's scope, is clean; a repo-wide run flags only the vendored
  `.specify/extensions/git/scripts/python/` files, which this change does not touch.)*
