...
- **The remote branch exists but is ahead of the local one.** Someone else pushed to it. Nothing on
  the local branch is unpublished, so deleting it would in fact be safe — but "would be safe" and
  "can be shown safe" are different, and only the second may authorise a delete. Showing it means
  asking whether the local commits are reachable from what the remote reported, which needs that
  commit to be present locally, and it usually is not: nobody has fetched it. So this case is
  **kept**, on the honest grounds that containment could not be established without fetching
  objects — which this check deliberately does not do. It is the one retention this feature adds
  that a perfect oracle would not, and the plan records it under Risks. When the clone does happen
  to hold the commit, the existing "commits ahead" question answers it and the branch is deleted.
...
- **The remote has the branch, and the base fetch failed.** Containment must not be provable by the
  ...
  asked.
- **Whatever asks the remote must not disturb the clone.** Cleanup runs against the author's own
  clone, and a design that made the local refs current in order to read them would be writing to
  that clone to answer a question about somewhere else — no local branch, no working tree, no
  checked-out ref, and no remote-tracking ref may move. This is stated as a constraint on any
  candidate design rather than a description of one; the design chosen writes nothing at all.
