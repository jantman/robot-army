...
| `trello.evaluated` | Per card, per evaluation | Whether it resolved, to what, the candidates it considered, and `source` — `declaration` if a `robot-army:` line on the card decided it, `scan` if the card's text did, `onboarding` if nothing was onboarded and so nothing on the card could have resolved (issue #83) |
