...
- [ ] T001 Confirm the baseline is green before touching anything: `uv run pytest`
...
- [ ] T002 Add `SCHEMA_013_SQL` and `_migration_013` to `src/robot_army/migrations.py`, adding
...
- [ ] T003 [P] Add `pull_requests: str | None = None` and `pull_requests_at: str | None = None`
...
- [ ] T004 [P] Add `tests/unit/test_migrations.py` coverage: migration 013 applied to a
      populated pre-013 database leaves existing rows readable with both columns `NULL`, and
      `SCHEMA_VERSION` is 13

### The boundary read

- [ ] T005 In `src/robot_army/boundaries/github.py`, give `GitHubReader._graphql` a
...
- [ ] T006 In `src/robot_army/boundaries/github.py`, add the `_PULL_REQUESTS` GraphQL document
      ...
      disappears
- [ ] T007 Replace `open_pr_for_branch` with `pull_requests_for` in the `IssueSourceReader`
...
- [ ] T008 Replace `open_pr_for_branch` with `pull_requests_for` on the fake reader in
      `tests/conftest.py`, defaulting to returning `[]`
- [ ] T009 Add `tests/unit/test_pull_requests.py` covering C1: a pull request found by the
...
- [ ] T010 [US1] Add `_refresh_pull_requests(conn, *, boundaries, audit)` to
      `src/robot_army/reconcile.py` implementing contracts C2: the five candidate rules in
      order, a per-pass cache on `(repo_key, issue_number, branch)`, and a returned count of
      items whose set changed. Docstring why the candidate rule needs no interval and no cap —
      it terminates itself once every stored pull request is `merged` or `closed` (R4)
- [ ] T011 [US1] Implement C3's write inside `_refresh_pull_requests`: serialise with
      `json.dumps(..., separators=(",", ":"))`; when the text equals what is stored, advance
      `pull_requests_at` alone and record nothing; when it differs, in one `db.transaction`
      write the `work_item.pull_requests` audit record *then*
      `db.update_work_item_columns`, following `speckit.record_phase`'s order. Catch
      `TransportError` only, log `reconcile.pull_requests_check` at `error`, and write neither
      column
- [ ] T012 [US1] Call `_refresh_pull_requests` from `reconcile.reconcile()` **before**
...
- [ ] T013 [US1] Add `pull_request_view(item) -> dict` to `src/robot_army/operations.py`
...
- [ ] T014 [US1] In `src/robot_army/operations.py`, remove the `open_pr_for_branch` call and the
...
- [ ] T015 [US1] Add a `pull request` row to the `<dl>` in `pages.item_view`
...
- [ ] T016 [US1] Carry `pull_requests`, `pull_requests_at` and `pull_requests_known` in
      `item_view`'s `View.data` under the same names the HTML renders, so the JSON and the HTML
      cannot drift (FR-017)

### Tests

- [ ] T017 [P] [US1] Add `tests/unit/test_reconcile_pull_requests.py` covering C2's candidate
      ...
      lookup
- [ ] T018 [P] [US1] Extend `tests/unit/test_reconcile_pull_requests.py` with C3's failure and
      ...
      untouched
- [ ] T019 [P] [US1] Add `pull_request` cases to `tests/unit/test_web_pages.py` asserting all
...
- [ ] T020 [US1] Update `tests/unit/test_resume_signals.py` for T014: `remote_resume_signals`
...
- [ ] T021 [US2] Add a compact `_pull_request_badge` helper to `src/robot_army/web/pages.py`
...
- [ ] T022 [US2] Add a `PR` column to `pages.active_view` after `spec-kit`, sourcing it from
      `pull_request_view` on each row, and carry the three keys in each row of `View.data`
- [ ] T023 [US2] Leave `/queue` without a column, and say so in a comment in `pages.queue_view`:
...
- [ ] T024 [P] [US2] Extend `tests/unit/test_web_pages.py`: `/active` renders a link for an item
...
- [ ] T025 [US3] Replace the `open PR` row in `pages._signals_cell` with a `pull requests` row
      ...
      own age (FR-019)
- [ ] T026 [US3] Update `pages._signal_row` to source the pull request from `pull_request_view`
      rather than from `remote_resume_signals`, and carry the three keys in the row it returns
- [ ] T027 [P] [US3] Add an `unreachable` case to `tests/unit/test_web_pages.py`: render
      ...
      (SC-004, C5)
- [ ] T028 [P] [US3] Extend `tests/unit/test_web_pages.py` for `/interrupted`: the three states
...
- [ ] T029 [P] Update [`docs/guide/operating.md`](../../docs/guide/operating.md): the web
...
- [ ] T030 [P] Update [`docs/guide/state.md`](../../docs/guide/state.md): `work_items` gains
...
- [ ] T031 [P] Update [`docs/guide/audit-log.md`](../../docs/guide/audit-log.md): the
...
- [ ] T032 Confirm no configuration key was added, so `exampleconfig.py` and
      `share/config.example.toml` need no regeneration and
      `tests/unit/test_example_config_drift.py` stays green
- [ ] T033 Run the full suite: `uv run pytest`. It must pass before the feature is complete
- [ ] T034 Walk [quickstart.md](./quickstart.md) §5 once against real GitHub with `gh api
