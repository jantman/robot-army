...
- [X] T001 Add the module constant `NOTHING_ONBOARDED` (the reason text from
...
- [X] T002 [P] [US1] Tests in `tests/unit/test_nothing_onboarded.py`: doctor fails the check
...
- [X] T003 [US1] In `operations.doctor` in `src/robot_army/operations.py`, compute
...
- [X] T004 [P] [US2] Tests in `tests/unit/test_nothing_onboarded.py`: scan path and
...
- [X] T005 [US2] In `resolve_repository` in `src/robot_army/intake.py`, return
...
- [X] T006 [US2] In `_needs_info_comment` in `src/robot_army/intake.py`, return the
  nothing-onboarded comment from the contract when `reason == NOTHING_ONBOARDED`.
- [X] T007 [US2] In the held-card activity gate in `src/robot_army/intake.py` (step 5 of card
...
- [X] T008 [US2] Run `uv run pytest tests/unit/test_repo_resolution.py
...
- [X] T009 [P] [US3] Extend the "With the database lost entirely" row in
...
- [X] T010 [P] [US3] Replace the "work items are also gone — that is expected" aside in
...
- [X] T011 [P] `docs/guide/1-setup.md`: after "run this first, every time", say `doctor` fails on
  `onboarded repositories` until the first `onboard`, and that this is expected.
- [X] T012 [P] `docs/guide/2-intake.md`: in "When a card doesn't say enough", describe the
  ...
  `doctor` says so.
- [X] T013 [P] `docs/guide/audit-log.md`: add `onboarding` to the `trello.evaluated` `source`
  values.
- [X] T014 Run `uv run pytest` (whole suite) and `uv run ruff check` if configured.
