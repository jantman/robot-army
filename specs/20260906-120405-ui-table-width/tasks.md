...
- [X] T001 Confirm the baseline: run `uv run pytest` and record that the suite passes before any edit
- [X] T002 Read `APP_CSS` in `src/robot_army/web/html.py` end to end, noting the four declarations the
...
- [X] T003 Render the current pages to files for a before/after comparison: seed three active items with
...
- [X] T004 Add `--measure: 60rem` and `--page: 120rem` to the `:root` block in `APP_CSS` in
      ...
      narrowed by it
- [X] T005 Add the two values to the dark and light palettes correctly — they are lengths, not colours,
...
- [X] T006 [US1] Create `tests/unit/test_web_layout.py` with a module docstring explaining why the
      stylesheet is asserted as a string: it is a module constant with no other seam, and the rules in
      it are the entire feature
- [X] T007 [US1] In `tests/unit/test_web_layout.py`, assert against `html.APP_CSS` that the prose measure
...
- [X] T008 [US1] In `tests/unit/test_web_layout.py`, assert that `main` is bounded by the page value and
...
- [X] T009 [US1] In `tests/unit/test_web_layout.py`, assert that the prose selector list caps paragraphs,
...
- [X] T010 [US1] Run the three new tests and confirm all three fail against the unmodified stylesheet

### Implementation for User Story 1

- [X] T011 [US1] Change `main` in `APP_CSS` in `src/robot_army/web/html.py` from `max-width: 60rem` to
...
- [X] T012 [US1] Add the prose rule to `APP_CSS` — `main p, main ul, main dl, main .banner, main .card,
...
- [X] T013 [US1] Run `uv run pytest tests/unit/test_web_layout.py` and confirm the three tests now pass
- [X] T014 [US1] Re-render `/active` and compare against the Phase 1 capture at a 1920-pixel window:
...
- [X] T015 [P] [US2] In `tests/unit/test_web_layout.py`, assert that `.scroll` is shrink-to-fit —
      `width: fit-content` and `max-width: 100%` — so a table takes the width its content needs rather
      than the width it is given
- [X] T016 [P] [US2] In `tests/unit/test_web_layout.py`, assert that `.scroll` still carries
...
- [X] T017 [US2] In `tests/unit/test_web_layout.py`, assert against rendered markup — through the `web`
...
- [X] T018 [US2] Run the new tests and confirm the two stylesheet assertions fail against the
...
- [X] T019 [US2] Add `width: fit-content; max-width: 100%;` to the `.scroll` rule in `APP_CSS` in
...
- [X] T020 [US2] Run `uv run pytest tests/unit/test_web_layout.py` and confirm every test passes
- [X] T021 [US2] Re-render `/queue`, `/cards` and an item page and compare against the Phase 1 capture:
...
- [X] T022 [US2] Check the `/queue` repositories table in particular — it is the one nested a level
...
- [X] T023 [US3] In `tests/unit/test_web_layout.py`, assert that no rule introduced by this feature can
...
- [X] T024 [US3] In `tests/unit/test_web_layout.py`, assert that `th { white-space: nowrap }` survives —
...
- [X] T025 [US3] Run the whole layout module and confirm the results are as expected

### Verification for User Story 3

- [X] T026 [US3] Verify in a real browser at a genuine 390-pixel viewport, not a resized desktop window —
...
- [X] T027 [US3] Measure with and without the change and confirm every number is identical: page
...
- [X] T028 [US3] Check a wide table by dragging it at 390 pixels: it scrolls inside its own box while the
...
- [X] T029 Update `docs/guide/operating.md` — the guide page for the web interface, per CLAUDE.md §2 —
...
- [X] T030 [P] Confirm no configuration key changed, so `exampleconfig.py` and
...
- [X] T031 [P] Confirm `README.md` is untouched and still under its 150-line limit
- [X] T032 Delete the scratch render helper from Phase 1 if it was added under `tests/`; it is a
      development aid, not a test, and must not be committed
- [X] T033 Run the full suite: `uv run pytest`. It must pass
- [X] T034 Run `uv run ruff check` and `uv run ruff format --check` if configured, and fix anything raised
- [X] T035 Walk [quickstart.md](./quickstart.md) end to end as written, on both viewports, and correct it
      if any step or number no longer matches what the code does
- [X] T036 Re-read the [layout contract](./contracts/layout.md) against the final stylesheet and confirm
...
Two places where what was done differs from what was written above. Both are recorded rather than
quietly absorbed.

**The tests were written before Phase 2, not after it.** As ordered, Phase 2 adds the two custom
properties and Phase 3 then writes a test asserting they exist — which would have been a test that
passed the moment it was written, against a stylesheet already carrying the thing it tests. All the
tests were written first instead, against the untouched stylesheet, and six of the fourteen failed:
the six that describe new behaviour. The other eight passed, and were meant to — they guard what this
feature must not break, and a guard that fails before the change would be describing a bug, not a
guard.

**T017 covers `/cards` through its own fixture.** The `web` fixture has no Trello board configured, and
without one the cards view renders a note where its tables would be. Parametrising `/cards` alongside
the other three routes would have produced a test that asserted nothing and passed. It gets a separate
test using `board_web`, which does have a board.

Two smaller things worth naming:

- **T034** found 25 lint errors and 147 unformatted files across the repository. All are pre-existing;
  CI runs `ruff check src/ tests/`, which passes, and does not run `ruff format`. Reformatting 147 files
  this feature did not touch would be a different change wearing this one's commit message. Only the new
  test file was formatted.
- **T035** found the quickstart naming `robot-army web`. The command is `robot-army serve`. Fixed, along
  with the measurements, which are now the ones actually taken rather than the ones estimated during
  planning.

### What was measured

At a 1920-pixel window, table widths before → after:

| View | Table | Before | After |
|---|---|---:|---:|
| `/active` | ten columns | 928 | 1702 |
| `/queue` | ready, eight columns | 928 | 1131 |
| `/queue` | repositories, five columns | 928 | 351 |
| item page | session attempts, eight columns | 928 | 743 |
| item page | state history, two columns | 928 | 410 |

The two that got *narrower* are the point of the shrink-to-fit rule, not a failure of it: neither table
wants 1888 pixels, and the `/queue` repositories table is the one nested a level deeper that the
rejected grid design would have missed (T022).

At a 390-pixel viewport, across all seven views: `allIdentical: true`. Page horizontal scroll, content
width, table container width, banner width and table scrolling all match the pre-change values exactly.
