...
def record(n: int, **overrides) -> dict:
    base = {
        "ts": f"2026-08-24T00:{n // 60:02d}:{n % 60:02d}Z",
        "component": "daemon",
        "kind": "event",
...
def test_a_truncated_final_line_yields_records_plus_a_non_zero_skip_count(ctx, layout):
    write_log(layout, "2026-08-24", [record(n) for n in range(5)], truncate_last=True)
...
def test_the_skipped_count_reaches_the_page(web, layout):
    write_log(layout, "2026-08-24", [record(n) for n in range(3)], truncate_last=True)
...
def test_records_come_back_newest_first(ctx, layout):
    write_log(layout, "2026-08-24", [record(n) for n in range(5)])
...
def test_an_unknown_outcome_is_a_usage_error_not_an_empty_page(ctx, layout):
    ...
    write_log(layout, "2026-08-24", [record(0)])
...
def test_paging_returns_disjoint_pages_within_one_file(ctx, layout):
    write_log(layout, "2026-08-24", [record(n) for n in range(10)])
...
def test_paging_returns_disjoint_pages_across_a_file_boundary(ctx, layout):
    ...
    write_log(layout, "2026-08-23", [record(n, ts=f"2026-08-23T00:00:{n:02d}Z") for n in range(5)])
    write_log(layout, "2026-08-24", [record(n, ts=f"2026-08-24T00:00:{n:02d}Z") for n in range(5)])
...
def test_the_last_page_reports_no_more(ctx, layout):
    write_log(layout, "2026-08-24", [record(n) for n in range(3)])
...
def test_the_web_offers_the_next_page_when_there_is_one(web, layout):
    write_log(layout, "2026-08-24", [record(n) for n in range(150)])
    body = web.get("/log").text
...
def test_paging_preserves_the_active_filters(web, layout):
    write_log(layout, "2026-08-24", [record(n, entity_id=1) for n in range(150)])
...
def test_a_work_item_record_links_to_its_page(web, layout, conn):
    ...
    write_log(layout, "2026-08-24", [record(0, entity_id=item_id)])
...
def test_a_first_page_is_bounded_against_a_hundred_thousand_records(ctx, layout):
    ...
    in a loop.
    """
    write_log(layout, "2026-08-24", [record(n % 3600) for n in range(100_000)])
...
def test_the_page_size_is_bounded_even_when_a_caller_asks_for_more(ctx, layout):
    write_log(layout, "2026-08-24", [record(n % 3600) for n in range(5000)])
