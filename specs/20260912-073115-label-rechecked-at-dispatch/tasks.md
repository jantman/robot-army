...
- [X] T001 Add `NOT_LABELLED = "not_labelled"` to `HoldReason` directly after `HELD`. Add a
...
- [X] T002 Add a pure `label_hold(item, label) -> str | None`. It returns the detail string
...
- [X] T003 [US1] In `_hold_for`, splice the check in:
...
- [X] T004 [P] [US1] Create `tests/unit/test_label_hold.py` with these tests:
...
- [X] T005 [P] [US1] Add a test in `tests/integration/test_dispatch_capacity.py` covering
  ...
  `reason: "not_labelled"`.
- [X] T006 [US1] Confirm `launch_holds` is unchanged for `resume`/`restart`. Add one assertion
...
- [X] T007 [US2] Change the existing-row branch of `poll_repo`. When `existing.state` is
...
- [X] T008 [P] [US2] Add tests in `tests/unit/test_poll.py`:
...
- [X] T009 [US3] Add `warn_about_label(conn, audit, config) -> str | None` beside
...
- [X] T010 [P] [US3] Add tests in `tests/integration/test_daemon_loop.py`: startup with
...
- [X] T011 [P] In `docs/guide/3-selection.md`, add the new row to the reasons table (and fix
  ...
  to release them.
- [X] T012 [P] In `docs/guide/2-intake.md`, extend "Changing the label" to say that already
  queued items are held and how to release them, linking to the selection page.
- [X] T013 [P] Add `poll.labels_refreshed` and `daemon.label_warning` to
  `docs/guide/audit-log.md`, and note that `dispatch.at_capacity`'s `reason` may be
  `not_labelled`.
- [X] T014 Run `uv run pytest`; the whole suite must pass.
