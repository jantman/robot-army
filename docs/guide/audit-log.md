...
|--------|------|----------------|
| `github.project.discover` | Every resolution attempt. `ok` when a board resolves **and** when the repository simply has none — an absent board is a third state, not a failure, and recording it as one would put an error a minute in the log for the ordinary condition of most repositories | Candidate projects seen, the project and column chosen, and whether each was `discovered` or `configured`. On failure, the `reason` a surface shows — which is why "why is this repository not ordered by its board?" is answerable from the log as well as from `status` |
...
| `poll.board` | Once per repository per pass, **except** for a repository that has never had a board and does not have one now — that writes nothing at all | `ok` with the counts, `ok` with a `skipped` reason when the repository is in backoff, or `error` with the failure and the new backoff. A skip is recorded rather than silent — "the board was not read this pass" is a fact about the order being shown |
| `poll.board.fallback` | A read failed or a resolution was lost, and a previous snapshot exists | How old that snapshot is, and **which of two different things follows** — the two must not be confused. After a *transport* failure the repository stays governed by the stale snapshot, order and holds intact. After a *resolution* failure it becomes **ungoverned**: board order and off-column holds are released and it falls back to the configured order until resolution succeeds again. Both are permitted by FR-025; recording the wrong one would make the log evidence for something that did not happen |
...
and a permanently held capacity slot. Retirement is what ends it, and — because ending a
process cannot be undone from this side — the intent is on disk before the signal is sent.

| Record | When | Notable detail |
|--------|------|----------------|
| `session.retire` | **New** — once per session, immediately **before** anything is signalled | `item_id`, `session_id`, `pid`, `proc_start`, `idle_s` — how long the worker had been idle when the decision was taken — and `signal`, which of the two conditions authorised it: `merged_pull_request` or `quiet_period`. Written first on purpose, so a daemon that dies between the decision and the kill still leaves the reason for a process that is about to disappear |
| `session.terminate` | As before | Unchanged. Retirement is a new *caller* of the existing confirmed-termination path, not a new mechanism, so the rungs, the refusals and the escalation record exactly as they do for `cancel` |
...
**`signal` was added by issue #149, and `idle_s` alone is why.** Retirement originally had one
gate — 30 minutes of idleness — so `idle_s` implied the reason and nothing else needed saying.
It now has two, the merged pull request being the primary one, and an `idle_s` of 47 is either
a merge or a bug. Principle III's standard is reconstruction from the log without re-running
anything, and that cannot mean "and knowing which release wrote the line", so the condition is
recorded rather than inferred. `idle_s` stays and is still the truth about the worker; it is
simply no longer the *reason*. The settle reason on the session row names the same fact in
prose, so a row read on its own says why its worker was ended.
...
| A **retirement deferred because the worker was not idle yet, or idle without a merge and not for long enough** | The reconciliation loop runs every 60 seconds and a finished session may sit idle for hours, so recording "still busy" would write ~1,440 records a day carrying one bit. The condition is re-derivable at any instant from the worker's own registry entry and the item's stored pull requests, and the *decision that acts* — `session.retire` and everything after it — is logged in full. Both of retirement's deferrals are covered by this one line and neither writes anything, unchanged by issue #149. Precedent: the transcript check writes nothing for a session still inside its grace period, for the same reason. See the issue #138 section above |
...
| Record | When | Notable detail |
|--------|------|----------------|
| `worktree.remove` | `worktree remove <path>` — a worktree no work item claims | `entity_type: worktree`, `entity_id` and `target` the resolved path, `detail.by: path`. Refusals before git is reached: `refused_by` is `outside_root`, `claimed` (with `claimed_by_item`), `not_a_directory`, `listing_failed`, `not_a_worktree` or `live_worker` (with the worker's `pid`, `session_id` and `cwd`). After git: `git` or `directory_survived`. `forced_over_live_worker` is this form's `forced_over_live_session` |
