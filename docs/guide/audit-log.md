...
| Record | When | Notable detail |
|--------|------|----------------|
| `session.retire` | **New** — once per session, immediately **before** anything is signalled | `item_id`, `session_id`, `pid`, `proc_start`, and `idle_s`: how long the worker had been idle when the decision was taken. Written first on purpose, so a daemon that dies between the decision and the kill still leaves the reason for a process that is about to disappear |
